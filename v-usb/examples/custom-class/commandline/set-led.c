...
int main(int argc, char **argv)
{
    ...
    /* compute VID/PID from usbconfig.h so that there is a central source of
     * information */
    ...
    if (strcasecmp(argv[1], "status") == 0) {
        cnt = libusb_control_transfer(
            handle,
            LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_RECIPIENT_DEVICE |
                LIBUSB_ENDPOINT_IN,
            CUSTOM_RQ_GET_STATUS, 0, 0, (unsigned char *)buffer, sizeof(buffer),
            5000);
        if (cnt < 1) {
            if (cnt < 0) {
    ...
    } else if ((isOn = (strcasecmp(argv[1], "on") == 0)) ||
               strcasecmp(argv[1], "off") == 0) {
        cnt = libusb_control_transfer(
            handle,
            LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_RECIPIENT_DEVICE |
                LIBUSB_ENDPOINT_OUT,
            CUSTOM_RQ_SET_STATUS, isOn, 0, (unsigned char *)buffer, 0, 5000);
        if (cnt < 0) {
            fprintf(stderr, "USB error: %s\n", libusb_strerror(cnt));
        }
#if ENABLE_TEST
        ...
        for (i = 0; i < 50000; i++) {
            ...
            cnt = libusb_control_transfer(
                ...
                LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_RECIPIENT_DEVICE |
                    USB_ENDPOINT_IN,
                CUSTOM_RQ_ECHO, value, index, (unsigned char *)buffer,
                sizeof(buffer), 5000);
            if (cnt < 0) {
