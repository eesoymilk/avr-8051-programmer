...
usbMsgLen_t usbFunctionSetup(uchar data[8])
{
    ...
    static uchar dataBuffer[4]; /* buffer must stay valid when usbFunctionSetup
                                   returns */

    if (rq->bRequest ==
        CUSTOM_RQ_ECHO) { /* echo -- used for reliability tests */
...
int __attribute__((noreturn)) main(void)
{
     ...
     * wdt_disable(). On newer devices, the status of the watchdog (on/off,
     * period) is PRESERVED OVER RESET!
     */
